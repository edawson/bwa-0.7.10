...
OUTPUT=bwa_output_WHWT_mem


## Define vars here so we can run locally
refPath=/corral-repl/tacc/bio/STIR-data/usda
databaseFasta=umd.tar
query1=WHWT.86391.AP.03.2_Unique.fastq
query2=WHWT.86391.AP.03.1_Unique.fastq
numThreads="-t 4"
# Vars normally  passed in from outside
...
QUERY1_F=${QUERY1} #$(basename ${QUERY1})
...
if [[ "$IS_PAIRED" -eq 1 ]];
then
	QUERY2_F=${QUERY2} # $(basename ${QUERY2})
...
for M in input1 input2 temp .launcher #bin $QUERY1_F $QUERY2_F $REFERENCE_F $REFERENCE_F.* input1 input2 temp .launcher
do
	echo "Cleaning $M"
	rm -rf $M
done
